...
${FFMPEG} -f x11grab -draw_mouse 0 -s ${Xres}x${Yres} -framerate 25 -i ${DISPLAY} -vf "scale=${OUTPUTW}:${OUTPUTH}" -c:v h264_qsv -preset medium -profile baseline -aspect ${AR} -pix_fmt yuv420p -global_quality 30 -look_ahead_depth 0 -max_dec_frame_buffering 0 -bufsize 128k -f mpegts "tcp://127.0.0.1:12345?listen"
